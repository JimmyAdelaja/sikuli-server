  ...
  public static void main(String[] args) {
    ...
    app.post(
        ...
        ctx -> {
          ...
          try {
            ...
            screen.click(getRegionFromBody(body).findAllList(imagePattern).get(index));
    ...
    app.post(
        ...
        ctx -> {
          ...
          Map<String, Object> body = ctx.bodyAsClass(Map.class);
          String text = (String) body.get("text");
          // Match match = screen.existsText(text);
    ...
    app.post(
        ...
        ctx -> {
          ...
          int index = (int) body.get("index");
          try {
            // Match match = getRegionFromBody(body).findAllText(text).get(index);
            Match match = findText(text, index, getRegionFromBody(body));
            screen.click(match);
            ctx.result("Text '" + text + "' found and clicked");
          } catch (FindFailed e) {
            ctx.status(404).result("Text not found: " + e.getMessage());
          }
        });
